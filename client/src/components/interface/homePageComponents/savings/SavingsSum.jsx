...
export default function SavingsSum() {
  ...
  const downloadData = async () => {
    try {
      let result = await Axios.post(
        "" + API_URL + ":" + API_PORT + "/stats/savings/sum"
      );
      setSavingsData(result.data.Suma);
    } catch (err) {
  ...
  return (
    <div className="stats--container">
      <StatHeader header="Oszczędności - suma" icon="money" />
      <div className="stats--body flex">
        {savingsData ? (
          <StatSumText suma={savingsData} />
        ) : (
