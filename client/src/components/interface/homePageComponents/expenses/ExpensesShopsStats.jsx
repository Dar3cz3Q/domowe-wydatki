...
export default function ExpensesCategoriesStats() {
  ...
  const downloadData = async () => {
    try {
      let result = await Axios.post(
        "" + API_URL + ":" + API_PORT + "/stats/expenses/shops"
      );
      const tempData = [["Sklep", "Ilość"]];
      result.data.forEach((element) => {
        tempData.push([element.Nazwa, Number(element.Ilosc)]);
      });
      setShopsData(tempData);
    } catch (err) {
  ...
  return (
    <div className="stats--container">
      <StatHeader header="Sklepy" icon="chart-pie" />
