...
export default function ExpensesMethodsStats() {
  ...
  const downloadData = async () => {
    try {
      let result = await Axios.post(
        "" + API_URL + ":" + API_PORT + "/stats/expenses/methods"
      );
      const tempData = [["Metoda", "Ilość"]];
      result.data.forEach((element) => {
        tempData.push([element.Nazwa, Number(element.Ilosc)]);
      });
      setMethodsData(tempData);
    } catch (err) {
  ...
  return (
    <div className="stats--container">
      <StatHeader header="Płatności" icon="chart-pie" />
